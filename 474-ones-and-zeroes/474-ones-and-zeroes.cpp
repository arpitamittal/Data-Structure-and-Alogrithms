...
public:
   ...
   int solve(int i, int one, int zero, int &maxZero, int &maxOne, vector<string>& arr)
    {
        if(i >= arr.size()) 
            return 0;
        ...
        {
            ans1 = 1 + solve(i + 1, one + p.first, zero + p.second, maxZero, maxOne, arr);
